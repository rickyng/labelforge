...
def apply_labels(
    ...
    # Group labels by page number for processing.
    # Include both changed labels and labels that need re-insertion (e.g. after shape redaction).
    processed_by_page: dict[int, list[Label]] = defaultdict(list)
    for label in labels:
        if label.new_text is not None:
            processed_by_page[label.page].append(label)

    total_changed = sum(len(v) for v in processed_by_page.values())
    if total_changed == 0:
    ...
    try:
        ...
        for page_num, page_labels in sorted(processed_by_page.items()):
            page: fitz.Page = doc[page_num]
            logger.debug("Processing page %d: %d changes", page_num, len(page_labels))

            # --- Phase 1: add redaction annotations (only for truly changed text) ---
            for label in page_labels:
                # Skip redaction for re-insert labels (unchanged text after shape redaction)
                if not label.is_changed:
                    continue
        ...
        # Avoid "save to original must be incremental" when input == output
...
def _make_reinsert_label(comp) -> Label:
    """Create a Label for re-inserting unchanged text after redaction."""
    return Label(
        id=comp.id,
        page=comp.page,
        bbox=comp.bbox,
        original_text=comp.text or "",
        new_text=comp.text or "",  # same as original — just re-insert
...
def apply_from_components(
    ...
    from .shape_handler import apply_shape_fill_change
    ...
    # Apply shape fill changes FIRST — redaction removes overlapping text,
    # so shapes must be redrawn before text is (re-)inserted.
    shape_bboxes: list[tuple[int, tuple[float, ...]]] = []  # (page, bbox) for overlap check
    for comp, new_fill_color in shape_jobs:
        if not comp.shape_drawings:
            logger.warning("Shape %s has no drawing items for redraw — skipped", comp.id)
            continue
        apply_shape_fill_change(
            input_path=current_input,
            output_path=output_path,
            page_num=comp.page,
            bbox=comp.bbox,
            shape_drawings=comp.shape_drawings,
            new_fill_color=new_fill_color,
            fill_opacity=comp.fill_opacity or 1.0,
            stroke_color=comp.stroke_color,
            stroke_width=comp.stroke_width,
        )
        current_input = output_path
        shape_bboxes.append((comp.page, comp.bbox))
        changed += 1

    # Ensure text components that overlap with shapes get re-applied with the
    # full font pipeline, even if their value hasn't changed. Shape redaction
    # removes all content in the bbox, so these texts need high-quality re-insertion.
    existing_text_ids = {l.id for l in text_labels}
    for comp in cf.components:
        if comp.type != ComponentType.TEXT or comp.id in existing_text_ids:
            continue
        comp_rect = fitz.Rect(comp.bbox)
        for shape_page, shape_bbox in shape_bboxes:
            if comp.page == shape_page and comp_rect.intersects(fitz.Rect(shape_bbox)):
                text_labels.append(_make_reinsert_label(comp))
                existing_text_ids.add(comp.id)
                break

    # Also detect text-to-text overlap: if a changed text's redaction bbox
    # overlaps with an unchanged text component, re-insert it too.
    changed_bboxes = [(l.page, l.bbox) for l in text_labels if l.is_changed]
    for comp in cf.components:
        if comp.type != ComponentType.TEXT or comp.id in existing_text_ids:
            continue
        comp_rect = fitz.Rect(comp.bbox)
        for changed_page, changed_bbox in changed_bboxes:
            if comp.page == changed_page and comp_rect.intersects(fitz.Rect(changed_bbox)):
                text_labels.append(_make_reinsert_label(comp))
                existing_text_ids.add(comp.id)
                break
    ...
    all_comp_ids = {c.id for c in cf.components}
    mapping_name = detect_mapping(all_comp_ids)
    if mapping_name and mapping_name in BARCODE_REGION_MAP:
        from .barcode_handler import generate_barcode_image, replace_component_image
        from .component_models import BarcodeFormat as BFmt

        regions = BARCODE_REGION_MAP[mapping_name]
        for region_name, region_cfg in regions.items():
            text_source = region_cfg.get("text_source", "")
            # Check if the text_source is in changes and its value changed
            new_value = changes.get(text_source)
            if not new_value:
                continue

            region_bbox = tuple(region_cfg["bbox"])
            fmt_str = region_cfg.get("format", "ean13")
            try:
                region_fmt = BFmt(fmt_str)
            except ValueError:
                logger.warning("Unknown barcode format %s for region %s — skipped", fmt_str, region_name)
                continue

            logger.info("Applying barcode region %s: text_source=%s new_value=%s", region_name, text_source, new_value)

            # Strip non-digit characters for barcode value (e.g. "8 431729 573797" → "8431729573797")
            barcode_value = "".join(c for c in new_value if c.isdigit())

            # Determine pixel size from bbox
            w_pt = region_bbox[2] - region_bbox[0]
            h_pt = region_bbox[3] - region_bbox[1]
            size_px = (max(1, round(w_pt * 150 / 72)), max(1, round(h_pt * 150 / 72)))

            new_image_bytes = generate_barcode_image(barcode_value, region_fmt, size_px)

            doc = fitz.open(str(current_input))
            try:
                replace_component_image(doc, 0, region_bbox, new_image_bytes)
                if current_input.resolve() == output_path.resolve():
    ...
    if not text_labels and not barcode_jobs and not shape_jobs:
        shutil.copy2(input_path, output_path)
